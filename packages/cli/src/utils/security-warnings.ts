 ...
 */
export async function displaySecurityWarnings(): Promise<void> {
  ...
  console.log(
    chalk.dim("   • Anyone with your private key can steal your funds")
  );
  console.log(chalk.dim("   • NEVER share your private key with anyone\n"));

  console.log(chalk.yellow("2. BACKUP YOUR PRIVATE KEY"));
  console.log(
    chalk.dim("   • Store your private key in a secure location")
  );
  console.log(
    chalk.dim(
      "   • Consider using a password manager or hardware wallet"
    )
  );
  ...
  console.log(
    chalk.dim(
      "   • Your .env file contains sensitive private key information"
    )
  );
  console.log(
    chalk.dim("   • Ensure .env is in your .gitignore file")
  );
  console.log(
    chalk.dim("   • NEVER push .env to GitHub, GitLab, or any public repo\n")
  );
  ...
  console.log(
    chalk.dim("   • Always verify URLs before entering sensitive information")
  );
  console.log(
    chalk.dim("   • Be cautious of unsolicited messages or emails\n")
  );

  // Require user acknowledgment
  const answer = await inquirer.prompt<{ acknowledged: boolean }>([
    {
      type: "confirm",
      name: "acknowledged",
      message: chalk.bold(
        "I understand the security risks and will keep my private key secure"
      ),
      default: false,
    },
  ]);
 ...
 */
export function displayFinalSecurityReminders(envPath: string): void {
  ...
  console.log(
    chalk.yellow("• Make sure this file is in your .gitignore")
  );
  ...
  console.log(
    chalk.yellow("• Consider moving funds to a hardware wallet for long-term storage\n")
  );
}
